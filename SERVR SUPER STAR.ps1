...
function Get-RestartEventLogs {
    ...
    if (Test-Connection $ComputerName -Quiet -Count 2) {
        try {
            ...
            [System.Windows.MessageBox]::Show("Can't retrive event logs from server $ComputerName. Check permissions.", "Server not reachable")
        }
    }
    else {
        [System.Windows.MessageBox]::Show("Provided Server isn't ok!.", "Server unreachable")
    }
}

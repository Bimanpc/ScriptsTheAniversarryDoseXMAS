...
REM Use curl to get the location info from ipinfo.io and parse the JSON response using jq
for /f "tokens=* delims=" %%i in ('curl -s http://ipinfo.io/%ip% ^| jq -r ".country"') do set country=%%i
